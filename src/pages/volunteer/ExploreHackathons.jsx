...
export default function ExploreHackathons() {
  ...
  const { user } = useAuth()
  const { hackathons, isVolunteeringFor } = useData()

  return (
    <DashboardShell role="volunteer" title="Find Hackathons" subtitle="Every hackathon published by organizers — sign up to volunteer.">
      <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
        {hackathons.map((h) => {
          const isSignedUp = isVolunteeringFor(user?.id, h.id)
          return (
            <Card
              key={h.id}
              className="flex cursor-pointer flex-col transition-colors hover:border-white/20"
              onClick={() => navigate(`/dashboard/volunteer/hackathons/${h.id}`)}
            >
              ...
              <Button
                ...
                className={isSignedUp ? 'mt-5 w-full !border-volunteer/40 !text-volunteer hover:!bg-volunteer-soft' : 'mt-5 w-full'}
                onClick={(e) => {
                  e.stopPropagation()
                  navigate(`/dashboard/volunteer/hackathons/${h.id}`)
                }}
              >
                {isSignedUp ? (
                  <>
